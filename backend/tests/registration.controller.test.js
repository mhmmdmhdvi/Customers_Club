...
function controller(registrationService) {
  return createAuthController({ otpService: { createOtp: async () => {} }, registrationService });
}
...
test("register returns created member but never claims an authenticated session", async () => {
  ...
  const api = controller({ register: async (data) => { assert.deepEqual(data, payload); return user; } });
  const res = response();
  await api.register({ body: payload }, res);
  assert.equal(res.statusCode, 201);
  assert.deepEqual(res.body, { message: "Registration completed", user, authenticated: false, nextStep: "LOGIN" });
  assert.equal(res.headers["Cache-Control"], "no-store");
...
test("verified-phone controller-to-service flow creates one member with a fake transaction store", async (t) => {
  ...
  const db = createRegistrationDb({ otps: [{ id: 1, phone: "09121234567", code: "123456", used: false, expiresAt: new Date(1_120_000) }] });
